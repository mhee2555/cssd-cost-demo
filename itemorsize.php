<script>
    ...
    function checkRadioSetGraph(num) {
        ...
        if (num == 1) {
            ...
            $("#radio_check2").css("background-color", "#a9b2c1");

        } else {
            $("#radio_check1").css("background-color", "#a9b2c1");
                                    ...
                                    if (Size == '-') {
                                        ...
                                        if (UserEdit == '1') {
                                            var text_stu = '<div class="form-check form-check-inline" >' +
                                                '<i  title="ผูกโดยผู้ใช้" class="fa-sharp fa-solid fa-triangle-exclamation" style="font-size: 30px;color: #FFCC00;"></i>' +
    ...
    function ConfirmSaveUserEdit(ItemCode, Size, RowIDItemSize, qty, qty_set, IDhn_person) {
        ...
        swal({
            ...
            if (dismiss === 'cancel') {
                // alert("sdsd");
                $("input[type=radio][name=RadioNameSize_" + ItemCode + "]").prop('checked', false);
            }
        });
    }
